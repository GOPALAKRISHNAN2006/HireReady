...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const sessions = await ProctoringSession.find(query)
      .select(
        'sessionType startTime endTime duration riskScore riskLevel report.integrityStatus status'
      )
      .sort({ startTime: -1 })
...
router.get(
  ...
  validateObjectId('id'),
  asyncHandler(async (req, res) => {
    const session = await ProctoringSession.findById(req.params.id).populate(
      'candidate',
