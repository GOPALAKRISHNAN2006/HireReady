...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const analytics = await Analytics.findOne({ user: req.user._id }).select(
      'overallStats.totalInterviews overallStats.averageScore overallStats.currentStreak'
    );
    ...
    upload.single('avatar')(req, res, err => {
      if (err instanceof multer.MulterError) {
        if (err.code === 'LIMIT_FILE_SIZE') {
          return res
            .status(400)
            .json({ success: false, message: 'Image size should be less than 5MB.' });
        }
      ...
      if (err) {
        return res
          .status(400)
          .json({ success: false, message: err.message || 'Only image files are allowed!' });
      }
      next();
    });
  },
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const user = await User.findById(req.user._id).select(
      '-refreshToken -password -passwordResetToken -passwordResetExpires'
    );
