...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const recentInterviews = await Interview.find({
      ...
      .sort({ completedAt: -1 })
      .limit(5)
      .select('title type category overallScore completedAt createdAt responses');

    // Format response to match frontend model conventions
    const formattedRecentInterviews = recentInterviews.map(interview => ({
      ...
      createdAt: interview.completedAt || interview.createdAt || new Date(),
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const allAchievements = [
      ...
      {
        name: 'High Achiever',
        description: 'Score above 90% on 10 interviews',
        category: 'score',
        icon: '⭐',
      },
      {
      ...
      {
        name: 'Speed Demon',
        description: 'Complete interview under 10 minutes',
        category: 'speed',
        icon: '⚡',
      },
    ];
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const totalUsers = await Analytics.countDocuments({
