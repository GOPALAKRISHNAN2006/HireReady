...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const topics = await GDTopic.find(query).sort({ createdAt: -1 }).lean();
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const aiParticipants = AI_PERSONALITIES.sort(() => Math.random() - 0.5)
      .slice(0, 4)
      .map(p => ({
        name: p.name,
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const respondingAI =
      session.aiParticipants[Math.floor(Math.random() * session.aiParticipants.length)];
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = {
      ...
      contributions:
        session.userContributions?.map(c => ({
          content: c.content,
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const topic = await GDTopic.create(topicData);
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const topic = await GDTopic.findByIdAndUpdate(req.params.id, updateData, {
      new: true,
      runValidators: true,
    });
