...
const includeDemoUsers =
  process.env.NODE_ENV !== 'production' || process.env.INCLUDE_DEMO_USERS === 'true';
...
const coreUsers = [
  {
    ...
    password: process.env.DEFAULT_ADMIN_PASSWORD || 'DevAdminPassword123!',
    role: 'admin',
...
const sampleQuestions = [
  // DSA Questions
  {
    ...
    type: 'technical',
    expectedAnswer:
      'Arrays offer O(1) random access but O(n) insertion/deletion. LinkedLists offer O(1) insertion/deletion but O(n) access. Use arrays for frequent access, linked lists for frequent modifications.',
  ...
  },
  {
    ...
    type: 'technical',
    expectedAnswer:
      'Best/Average: O(n log n), Worst: O(n²) when pivot selection is poor. Randomized pivot selection can help avoid worst case.',
  ...
  },
  {
    ...
    type: 'coding',
    expectedAnswer:
      "Use Floyd's Cycle Detection (tortoise and hare). Two pointers, one moves 2x speed. If they meet, there's a cycle.",
  ...
  },
  {
    ...
    type: 'coding',
    expectedAnswer:
      'DP breaks problems into overlapping subproblems with optimal substructure. Fibonacci: use memoization or tabulation to achieve O(n) time vs O(2^n) recursive.',
  ...
  // Web Development Questions
  {
    ...
    type: 'conceptual',
    expectedAnswer:
      'REST uses multiple endpoints with fixed data structures. GraphQL uses single endpoint with flexible queries. GraphQL for complex data needs, REST for simpler APIs.',
  ...
  },
  {
    ...
    type: 'technical',
    expectedAnswer:
      'Virtual DOM is a lightweight copy of the real DOM. React compares changes (diffing) and updates only what changed (reconciliation), minimizing expensive DOM operations.',
  ...
  },
  {
    ...
    type: 'technical',
    expectedAnswer:
      'A closure is a function that remembers variables from its outer scope even after the outer function returns. Used for data privacy, callbacks, and maintaining state.',
  ...
  // System Design Questions
  {
    ...
    type: 'technical',
    expectedAnswer:
      'Use base62 encoding for short URLs, NoSQL for high write throughput, caching layer (Redis) for popular URLs, load balancer for distribution.',
  ...
  },
  {
    ...
    type: 'technical',
    expectedAnswer:
      'Algorithms: Token Bucket, Leaky Bucket, Fixed Window, Sliding Window. Consider distributed systems using Redis. Handle edge cases and configure per-user limits.',
  ...
  // Behavioral Questions
  {
    ...
    type: 'behavioral',
    expectedAnswer:
      'Use STAR method. Show empathy, communication skills, conflict resolution. Focus on professional outcome and lessons learned.',
  ...
  },
  {
    ...
    type: 'behavioral',
    expectedAnswer:
      'Should demonstrate technical skills, problem-solving, ownership, and impact. Include metrics if possible.',
  ...
  // Database Questions
  {
    ...
    type: 'conceptual',
    expectedAnswer:
      'SQL: structured data, ACID compliance, complex queries (MySQL, PostgreSQL). NoSQL: flexible schema, horizontal scaling, high throughput (MongoDB, Cassandra).',
  ...
  },
  {
    ...
    type: 'technical',
    expectedAnswer:
      'Indexes are data structures (B-trees usually) that speed up lookups. Trade-offs: slower writes, storage overhead. Choose based on query patterns.',
  ...
  // ML Questions
  {
    ...
    type: 'conceptual',
    expectedAnswer:
      'Bias: error from oversimplified assumptions (underfitting). Variance: error from sensitivity to training data (overfitting). Goal: find optimal balance.',
  ...
  },
  {
    ...
    type: 'technical',
    expectedAnswer:
      'Optimization algorithm to minimize loss. Variants: Batch (full dataset), Stochastic (single sample), Mini-batch (subset). Adam, RMSprop for adaptive learning rates.',
      ...
      for (const user of regularUsers) {
        ...
        for (let i = 0; i < numInterviews; i++) {
          ...
          await Interview.create({
            ...
            insights: {
              overallFeedback:
                'Overall solid performance. Continue practicing data structures and system design concepts.',
        ...
        console.log(
          `   ✓ Created ${numInterviews} interviews for ${user.firstName} ${user.lastName}`
        );
      }
    } else {
