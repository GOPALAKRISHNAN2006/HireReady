const Card = ({
  ...
  const getBaseStyles = () => {
    if (glass)
      return 'bg-white/60 dark:bg-slate-800/60 backdrop-blur-xl border border-white/30 dark:border-slate-700/30 shadow-xl';
    if (gradient)
      return 'bg-gradient-to-br from-white via-slate-50/80 to-white dark:from-slate-800 dark:via-slate-800/80 dark:to-slate-800 border border-slate-100/50 dark:border-slate-700/50';
...
const CardHeader = ({ children, className = '', noBorder = false, ...props }) => (
  <div
    className={`${noBorder ? '' : 'border-b border-slate-100/80 dark:border-slate-700/80'} pb-4 mb-4 ${className}`}
    {...props}
  >
    {children}
  </div>
...
const CardTitle = ({ children, className = '', gradient = false, ...props }) => (
  <h3
    className={`text-lg font-bold ${gradient ? 'bg-gradient-to-r from-slate-900 via-primary-800 to-slate-900 dark:from-white dark:via-primary-300 dark:to-white bg-clip-text text-transparent' : 'text-slate-900 dark:text-white'} ${className}`}
    {...props}
  >
    {children}
  </h3>
...
const CardFooter = ({ children, className = '', noBorder = false, ...props }) => (
  <div
    className={`${noBorder ? '' : 'border-t border-slate-100/80 dark:border-slate-700/80'} pt-4 mt-4 ${className}`}
    {...props}
  >
    {children}
  </div>
