...
const MarkdownText = ({ text }) => {
  const renderLine = (line, i) => {
    ...
    html = html.replace(
      /`([^`]+)`/g,
      '<code class="px-1 py-0.5 bg-slate-100 dark:bg-slate-700 rounded text-xs font-mono">$1</code>'
    );
    // Links
    html = html.replace(
      /\[([^\]]+)\]\(([^)]+)\)/g,
      '<a href="$2" target="_blank" rel="noopener" class="text-primary-500 underline">$1</a>'
    );
    return html;
  };
    ...
    if (listItems.length > 0) {
      elements.push(
        <ul key={`ul-${elements.length}`} className="list-disc list-inside space-y-1 my-1">
          {listItems.map((li, i) => (
            <li key={i} className="text-sm" dangerouslySetInnerHTML={{ __html: renderLine(li) }} />
          ))}
        </ul>
      );
      listItems = [];
    ...
    if (line.startsWith('```')) {
      if (inCodeBlock) {
        elements.push(
          <pre
            key={`code-${i}`}
            className="bg-slate-100 dark:bg-slate-800 rounded-lg p-3 my-2 overflow-x-auto text-xs font-mono"
          >
            <code>{codeLines.join('\n')}</code>
          </pre>
        );
...
const AIChatbot = ({ externalOpen, onExternalClose }) => {
  ...
  const getFallbackResponse = msg => {
    ...
    if (lower.includes('interview') || lower.includes('tip'))
      return 'Practice mock interviews regularly, use the STAR method for behavioral questions, and study DSA patterns. Check out our Interview Tips section!';
    if (lower.includes('dsa') || lower.includes('coding'))
      return 'Focus on arrays, trees, graphs, and dynamic programming. Try our Daily Challenges to build consistency!';
    if (lower.includes('resume'))
      return 'Use our Resume Builder to create an ATS-friendly resume. Quantify your achievements and use action verbs!';
    if (lower.includes('company') || lower.includes('faang'))
      return 'Visit Company Prep in the sidebar for detailed preparation guides for top tech companies!';
  ...
  return (
    <div
      className={`fixed bottom-6 right-6 z-50 ${isMinimized ? 'w-72' : 'w-[calc(100vw-3rem)] sm:w-96'} transition-all duration-300 max-w-[calc(100vw-3rem)]`}
    >
      <div className="bg-white dark:bg-[#0d1526] rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden flex flex-col max-h-[600px]">
        {/* Header */}
        <div className="bg-gradient-to-r from-primary-600 to-primary-600 p-4 text-white flex-shrink-0">
            ...
            >
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mt-1">
                        <span
                          className={`text-xs ${
                            message.type === 'user' ? 'text-primary-200' : 'text-slate-400'
                          }`}
                        >
              ...
              {isTyping && (
                <div className="flex justify-start">
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary-500 to-pink-500 flex items-center justify-center text-white">
                      <Bot className="w-4 h-4" />
                    </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {quickActions.map((action, index) => (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-[#151d2e] hover:bg-primary-50 dark:hover:bg-primary-900/30 hover:text-primary-600 rounded-full text-xs font-medium text-slate-600 dark:text-slate-300 transition-colors"
                    >
            ...
            <div className="p-4 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-[#0d1526] flex-shrink-0">
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  className="flex-1 px-4 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent text-sm bg-white dark:bg-[#151d2e] text-slate-900 dark:text-slate-100 placeholder:text-slate-400"
                />
                <button
                  onClick={handleSend}
