...
const SavedQuestions = () => {
  ...
  const categories = [
    { id: 'all', label: 'All', icon: Bookmark, count: savedData?.length || 0 },
    {
      ...
      label: 'DSA',
      icon: Code,
      count: savedData?.filter(q => q.category === 'dsa').length || 0,
    },
    {
      id: 'system-design',
      label: 'System Design',
      icon: Layers,
      count: savedData?.filter(q => q.category === 'system-design').length || 0,
    },
    {
      id: 'behavioral',
      label: 'Behavioral',
      icon: MessageSquare,
      count: savedData?.filter(q => q.category === 'behavioral').length || 0,
    },
    {
      id: 'database',
      label: 'Database',
      icon: Database,
      count: savedData?.filter(q => q.category === 'database').length || 0,
    },
    {
      id: 'devops',
      label: 'DevOps',
      icon: Cloud,
      count: savedData?.filter(q => q.category === 'devops').length || 0,
    },
    {
      id: 'ml',
      ...
      icon: Bot,
      count:
        savedData?.filter(q => q.category === 'ml' || q.category === 'machine-learning').length ||
        0,
    },
  ];
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent bg-white dark:bg-slate-800 text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500"
          />
        </div>
        <div className="flex items-center space-x-2">
          <select
            ...
            className="px-4 py-3 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent appearance-none bg-white dark:bg-slate-800 text-slate-900 dark:text-white cursor-pointer"
          >
            ...
            <div className="text-center py-12">
              ...
              <p className="text-slate-500 dark:text-slate-400">
                {searchQuery
                  ...
                  : 'Save questions from your interviews to practice later'}
              </p>
            </div>
          </Card>
        ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="flex items-center space-x-3 text-sm text-slate-500 dark:text-slate-400">
                        <span
                          className={`px-2 py-1 rounded-lg text-xs font-medium ${getDifficultyColor(question.difficulty)}`}
                        >
