...
const InterviewNotes = () => {
  ...
  const NoteCard = ({ note }) => {
    ...
    return (
      <div
        className={`${colors.bg} ${colors.border} border rounded-xl p-5 transition-all hover:shadow-lg group relative`}
      >
        {/* Pin indicator */}
        {note.pinned && (
          ...
          <>
            <div className="flex items-start justify-between mb-2">
              ...
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                >
                  {note.pinned ? (
                    <PinOff className="w-4 h-4 text-primary-500" />
                  ) : (
            ...
            {note.tags.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mb-3">
                {note.tags.map(tag => (
                  <span
                    key={tag}
                    className="px-2 py-0.5 bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 rounded-full text-xs font-medium"
                  >
                    #{tag}
                  </span>
                ))}
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-primary-500"
          />
        </div>
          ...
          <Card.Content className="p-6">
            ...
            <div className="space-y-4">
              <input
                ...
                placeholder="Note title..."
                className="w-full px-4 py-3 border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800 text-slate-900 dark:text-white font-semibold text-lg focus:ring-2 focus:ring-primary-500"
                autoFocus
              />
              <textarea
                ...
                rows={8}
                className="w-full px-4 py-3 border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm resize-none focus:ring-2 focus:ring-primary-500"
              />
        ...
        <div className="text-center py-16">
          ...
          <p className="text-slate-500 dark:text-slate-400 mb-4">
            {searchQuery
              ...
              : 'Start taking notes to organize your interview prep'}
          </p>
          {!searchQuery && (
            <Button onClick={() => setIsCreating(true)} icon={Plus}>
