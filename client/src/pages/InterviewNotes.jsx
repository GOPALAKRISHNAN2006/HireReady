        ...
        {isEditing ? (
          <div className="space-y-3">
            <input
              ...
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-white font-semibold"
              ...
              <div className="flex gap-1">
                {COLORS.map(color => (
                  <button
                    ...
                    className={`w-6 h-6 rounded-full ${color.bg} ${color.border} border-2 ${note.color === color.id ? 'ring-2 ring-primary-500' : ''}`}
                  />
                ))}
              </div>
            </div>
          ...
          <>
            <div className="flex items-start justify-between mb-2">
              ...
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => toggleStar(note.id)} className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg">
                  {note.starred ? <Star className="w-4 h-4 text-yellow-500 fill-yellow-500" /> : <StarOff className="w-4 h-4 text-gray-400" />}
                </button>
                <button onClick={() => togglePin(note.id)} className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg">
                  {note.pinned ? <PinOff className="w-4 h-4 text-primary-500" /> : <Pin className="w-4 h-4 text-gray-400" />}
                </button>
                <button onClick={() => setEditingNote(note.id)} className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 whitespace-pre-wrap line-clamp-6 mb-3">
            ...
            {note.tags.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mb-3">
                {note.tags.map(tag => (
                  <span key={tag} className="px-2 py-0.5 bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 rounded-full text-xs font-medium">
                    #{tag}
                  </span>
                ))}
              </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">Organize your interview prep notes</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500"
          />
        </div>
          ...
          <Card.Content className="p-6">
            ...
            <div className="space-y-4">
              <input
                ...
                placeholder="Note title..."
                className="w-full px-4 py-3 border border-gray-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white font-semibold text-lg focus:ring-2 focus:ring-primary-500"
                autoFocus
              />
              <textarea
                ...
                rows={8}
                className="w-full px-4 py-3 border border-gray-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm resize-none focus:ring-2 focus:ring-primary-500"
              />
                  ...
                  <div className="flex gap-2">
                    {COLORS.map(color => (
                      <button
                        ...
                        className={`w-7 h-7 rounded-full ${color.bg} ${color.border} border-2 ${newNote.color === color.id ? 'ring-2 ring-primary-500 ring-offset-2' : ''}`}
                      />
                    ))}
                  </div>
                </div>
                <div className="flex-1 min-w-[200px]">
                  <label className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1 block">Tags</label>
                  <div className="flex gap-2">
                    <input
                      value={tagInput}
                  ...
                  {newNote.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {newNote.tags.map(tag => (
                        <span key={tag} className="inline-flex items-center gap-1 px-2 py-0.5 bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 rounded-full text-xs">
                          #{tag}
      ...
      {notes.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 text-center">
            <p className="text-2xl font-bold text-primary-500">{notes.filter(n => n.pinned).length}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400">Pinned</p>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 text-center">
            <p className="text-2xl font-bold text-green-500">{new Set(notes.flatMap(n => n.tags)).size}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400">Tags Used</p>
          </div>
        </div>
      )}
    </div>
