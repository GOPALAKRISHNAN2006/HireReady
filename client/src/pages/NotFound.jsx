...
const NotFound = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 flex items-center justify-center px-4 relative overflow-hidden">
      ...
      />
      <div
        className="absolute bottom-20 right-10 w-96 h-96 bg-primary-500/10 rounded-full blur-3xl transition-transform duration-700 ease-out"
      ...
      {[...Array(6)].map((_, i) => (
        <div
          key={i}
          className="absolute w-2 h-2 bg-primary-400/30 dark:bg-primary-400/20 rounded-full animate-bounce"
          style={{
            top: `${15 + i * 15}%`,
            left: `${10 + i * 16}%`,
      ...
      <div className="relative z-10 text-center max-w-2xl mx-auto">
        ...
        <div className="relative mb-8">
          ...
          >
            <span className="bg-gradient-to-br from-primary-500 via-primary-500 to-pink-500 bg-clip-text text-transparent drop-shadow-sm">
              4
            </span>
            <span className="relative inline-block">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div
                  className="w-28 h-28 md:w-36 md:h-36 border-4 border-dashed border-primary-300/40 dark:border-primary-500/30 rounded-full animate-spin"
        ...
        <p className="text-lg text-slate-600 dark:text-slate-400 mb-2 max-w-md mx-auto">
          The page you're looking for has drifted into another dimension. Let's get you back on
          track!
        </p>
        ...
        <form onSubmit={handleSearch} className="max-w-md mx-auto mb-10">
          <div className="relative group">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-4 bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 transition-all shadow-lg shadow-slate-200/50 dark:shadow-slate-900/50"
            />
          </div>
        </form>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center mb-12">
          <Link to="/">
            <Button
              icon={Home}
              size="lg"
              className="shadow-lg shadow-primary-500/30 hover:shadow-xl hover:shadow-primary-500/40 transition-shadow"
            >
              Go to Home
            </Button>
          </Link>
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {popularPages.map(page => (
              <Link
                key={page.path}
                to={page.path}
                className="group flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:border-primary-300 dark:hover:border-primary-600 hover:shadow-lg transition-all duration-200"
              >
                <div
                  className={`w-7 h-7 bg-gradient-to-br ${page.color} rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform`}
                >
