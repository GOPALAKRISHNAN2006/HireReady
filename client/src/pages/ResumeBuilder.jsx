...
const ResumeBuilder = () => {
  ...
  const templates = [
    ...
    { id: 'professional', name: 'Professional', color: 'bg-primary-500' },
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-cyan-600 via-teal-600 to-cyan-600 p-8 text-white">
        ...
        <div className="relative">
          ...
          <p className="text-white/70 max-w-lg">
            Create professional resumes tailored for your dream job
          </p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {templates.map(template => (
            <button
              ...
              onClick={() => {
                api
                  .post('/resumes', { title: `${template.name} Resume`, template: template.id })
                  .then(res => setResumes([res.data.data, ...resumes]));
              }}
              className="p-4 border dark:border-slate-700 rounded-lg hover:border-primary-500 hover:shadow-md transition-all group"
            >
              <div
                className={`w-full h-24 ${template.color} rounded mb-2 flex items-center justify-center`}
              >
