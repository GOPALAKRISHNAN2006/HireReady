...
const CommunityHub = () => {
  ...
  const bookmarkPostMutation = useMutation({
    mutationFn: id =>
      communityApi.bookmarkPost ? communityApi.bookmarkPost(id) : Promise.resolve(),
    onSuccess: () => {
  ...
  const deleteCommentMutation = useMutation({
    mutationFn: ({ postId, commentId }) => communityApi.deleteComment(postId, commentId),
    onSuccess: () => {
      queryClient.invalidateQueries(['community', 'feed']);
  ...
  const trendingTopics =
    trendingData?.popularTags?.map(t => ({
      tag: t.tag,
      count: `${t.count} posts`,
    })) || [];
                    ...
                    <div className="flex gap-4">
                      <img
                        ...
                        onError={e => {
                          ...
                          e.target.src = getAvatarUrl(null, post.author);
                        }}
                      />
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-6 pt-4 border-t border-slate-100 dark:border-slate-700">
                          <button
                            ...
                            className={`flex items-center gap-2 text-sm transition-colors ${post.isLiked ? 'text-red-500' : 'text-slate-500 hover:text-red-500'}`}
                          >
              ...
              {(discussionsData || []).length > 0 ? (
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-bold text-slate-900 dark:text-white mb-1">
                          {discussion.title || discussion.topic}
                        </h3>
                ...
                <Card>
                  <div className="text-center py-12">
                    ...
                    <p className="text-slate-600 dark;text-slate-400">
                      Start a discussion by creating a post!
                    </p>
                  </div>
                </Card>
              )}
              ...
              {posts.length > 0 ? (
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-4 pt-3 border-t border-slate-100 dark:border-slate-700">
                          <button
                            onClick={() => likePostMutation.mutate(post.id)}
                            className={`flex items-center gap-2 text-sm ${post.isLiked ? 'text-red-500' : 'text-slate-500 hover:text-red-500'}`}
                          >
                ...
                <Card>
                  <div className="text-center py-12">
                    ...
                    <p className="text-slate-600 dark:text-slate-400">
                      Share your interview success with the community!
                    </p>
                  </div>
                </Card>
              )}
                  ...
                  <Card key={index} hover>
                    <div className="flex flex-col sm:flex-row items-start gap-4">
                      <img
                        ...
                        onError={e => {
                          e.target.onerror = null;
                          e.target.src = getAvatarUrl(null, mentor.name);
                        }}
                      />
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
