    ...
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-sky-600 via-blue-600 to-sky-600 p-8 text-white">
        ...
        <div className="relative">
          ...
          <p className="text-white/70 max-w-lg">Find answers to common questions and get assistance</p>
        </div>
      </div>
      ...
      {/* Popular Topics */}
      <div>
        <h2 className="text-sm font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-3 text-center">Popular topics</h2>
        <div className="flex flex-wrap justify-center gap-2">
          {popularTopics.map((topic, i) => (
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-full text-sm font-medium text-slate-700 dark:text-slate-300 hover:border-indigo-300 dark:hover:border-indigo-600 hover:shadow-md transition-all duration-200"
            >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {resources.map((resource, index) => (
          <a
            key={index}
            href={resource.link}
            className="group p-5 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 hover:border-indigo-200 dark:hover:border-indigo-700 hover:shadow-lg hover:-translate-y-1 transition-all duration-300"
          >
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{filteredFaqs.length} questions found</p>
          </div>
        </div>
            ...
            filteredFaqs.map((faq) => (
              <div
                ...
                className={`bg-white dark:bg-slate-800 rounded-2xl border transition-all duration-200 ${
                  expandedFaq === faq.id
                    ? 'border-indigo-200 dark:border-indigo-700 shadow-md shadow-indigo-500/5'
              ...
              >
                ...
                >
                  ...
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 transition-all duration-200 ${
                    expandedFaq === faq.id ? 'bg-indigo-100 dark:bg-indigo-900/30 rotate-180' : 'bg-slate-100 dark:bg-slate-700'
                  }`}>
                    <ChevronDown className={`w-4 h-4 ${expandedFaq === faq.id ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400'}`} />
                  </div>
                </button>
                {expandedFaq === faq.id && (
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 dark:from-indigo-900/20 dark:via-purple-900/20 dark:to-pink-900/20 rounded-3xl p-8 md:p-12 border border-indigo-100 dark:border-indigo-800/50">
        <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-indigo-400/10 to-purple-400/10 rounded-full -translate-y-1/2 translate-x-1/2" />
        <div className="relative text-center">
          ...
          <p className="text-slate-600 dark:text-slate-400 mb-8 max-w-md mx-auto">Our support team typically responds within 2 hours during business days</p>
          <div className="flex flex-col sm:flex-row justify-center gap-3">
            <button 
              ...
              className="inline-flex items-center justify-center gap-2 px-8 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-medium rounded-xl hover:from-indigo-700 hover:to-purple-700 shadow-lg shadow-indigo-500/30 transition-all hover:scale-105 hover:-translate-y-0.5"
            >
