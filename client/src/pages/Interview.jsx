...
const Interview = () => {
      ...
      if (interview && interview.responses && interview.responses.length > 0) {
        interview.questions = interview.responses.map((r, index) => {
          ...
          if (!questionText) {
            ...
            questionText =
              fallbackQuestions[interview.category] ||
              `Interview Question ${index + 1}: Describe your approach to problem-solving.`;
          }
          return {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = e => {
      if (isActive && e.key === 'Escape') {
        e.preventDefault();
        e.stopPropagation();
        toast.error('Cannot exit fullscreen during interview. Complete the interview first.', {
          id: 'no-escape',
        });
        return false;
      }
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      if (newTime === 60) {
        toast.error('Only 1 minute remaining! Your interview will auto-submit.', {
          icon: '⏰',
          id: 'time-warning-1',
          duration: 10000,
        });
          ...
          completeInterviewMutation.mutate(undefined, {
            onSuccess: data => {
              ...
              setResultData({
                ...
                questionsAnswered:
                  result.questionsAnswered || responses.filter(r => r?.answer).length,
    ...
    onSuccess: data => {
      ...
      setResultData({
        overallScore:
          result.overallScore ||
          Math.round(
            (responses.filter(r => r?.answer).length / (currentInterview?.questions?.length || 1)) *
              100
          ),
  ...
  const handleSubmitAnswer = useCallback(() => {
    ...
    const questionId =
      typeof currentQuestion._id === 'object'
      ...
      },
      {
        onSuccess: data => {
          ...
          if (data?.data?.evaluation?.overallScore) {
            toast.success(`Answer submitted! Score: ${data.data.evaluation.overallScore}%`, {
              duration: 3000,
            });
          } else {
  ...
  const InterviewContent = (
    <div className="fixed inset-0 min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-primary-50 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800 overflow-auto z-40">
        ...
        <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-lg rounded-2xl shadow-lg border border-white/50 dark:border-slate-700/50 p-3 sm:p-5 mb-4 sm:mb-8 sticky top-4 z-30">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
            ...
            <div className="flex items-center space-x-4">
              {timeRemaining !== null && (
                <div
                  className={`flex items-center space-x-2 px-4 py-2.5 rounded-xl font-mono text-lg ${
                    timeRemaining < 300
        ...
        <div className="flex gap-3 mb-8 overflow-x-auto pb-3 scrollbar-hide px-1">
          {currentInterview?.questions?.map((_, index) => {
            ...
            return (
              ...
              >
                {hasAnswer && !isCurrent ? <CheckCircle className="w-5 h-5 mx-auto" /> : index + 1}
              </button>
            );
          })}
        </div>
        ...
        <div className="grid lg:grid-cols-4 gap-4 lg:gap-8">
          ...
          <div className="lg:col-span-3">
            <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-lg rounded-3xl shadow-xl border border-white/50 dark:border-slate-700/50 overflow-hidden">
              {/* Question Header */}
              <div className="bg-gradient-to-r from-primary-500 to-primary-500 px-4 py-4 md:px-8 md:py-6">
                  ...
                  <div className="flex items-start space-x-4">
                    ...
                    <h2 className="text-xl font-semibold text-slate-900 dark:text-white leading-relaxed flex-1">
                      {currentQuestion?.text ||
                        (currentInterview?.category === 'behavioral'
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative">
                    <textarea
                      ...
                      rows={12}
                      className="w-full px-6 py-5 bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-600 rounded-2xl focus:ring-4 focus:ring-primary-500/20 focus:border-primary-500 transition-all duration-300 resize-none text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500"
                    />
                    <div className="absolute bottom-4 left-6 right-6 flex justify-between text-sm">
                      <span
                        className={`font-medium ${answer.length > 100 ? 'text-green-500' : 'text-slate-400'}`}
                      >
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            {currentQuestion?.hints?.length > 0 && (
              <div className="bg-gradient-to-br from-blue-50 to-primary-50 rounded-2xl shadow-lg border border-blue-100/50 p-6">
                ...
                <ul className="space-y-2 text-sm text-blue-800">
                  {currentQuestion.hints.map((hint, i) => (
                    <li key={i} className="flex items-start">
                      <span className="w-5 h-5 bg-blue-200 rounded-full text-xs flex items-center justify-center mr-2 mt-0.5 flex-shrink-0">
                        {i + 1}
                      </span>
                      <span>{hint}</span>
                    </li>
        ...
        >
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-4">
              ...
              <div className="bg-gradient-to-br from-primary-50 to-primary-100 rounded-xl p-3 sm:p-4 text-center">
                <div className="text-2xl font-bold text-primary-600">
                  {resultData?.totalDurationSeconds
                    ? `${Math.floor(resultData.totalDurationSeconds / 60)}m`
                    : 'N/A'}
                </div>
                <div className="text-xs text-primary-700">Duration</div>
              </div>
            </div>
            ...
            <div
              className={`p-4 rounded-xl text-center ${
                (resultData?.overallScore || 0) >= 80
                  ...
                  : (resultData?.overallScore || 0) >= 60
                    ? 'bg-gradient-to-r from-blue-100 to-primary-100 text-blue-800'
                    : (resultData?.overallScore || 0) >= 40
            ...
            >
              ...
              <p className="text-sm mt-1 opacity-80">
                {resultData?.feedback?.summary ||
                  'Check the detailed results for feedback and improvement areas.'}
              </p>
            </div>
          </div>
