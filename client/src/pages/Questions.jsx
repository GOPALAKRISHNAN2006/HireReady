      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-primary-600 via-primary-600 to-primary-600 p-8 text-white">
        ...
        <div className="relative">
          ...
          <p className="text-white/70 max-w-lg">
            Browse and practice interview questions across multiple categories and difficulty levels
          </p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-700/60 shadow-sm p-5">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex-1 relative">
            <div className="absolute left-4 top-1/2 -translate-y-1/2 w-9 h-9 bg-gradient-to-br from-primary-500 to-primary-500 rounded-xl flex items-center justify-center shadow-lg shadow-primary-500/20">
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none w-full md:w-48 px-4 py-3 pr-10 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary-500/20 focus:border-primary-400 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white transition-all duration-200"
            >
              {categories.map(cat => (
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none w-full md:w-40 px-4 py-3 pr-10 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary-500/20 focus:border-primary-400 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white transition-all duration-200"
            >
        ...
        {isLoading ? (
          ...
          data.questions.map(question => {
            ...
            return (
              <div
                key={question._id}
                className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-700/60 shadow-sm hover:shadow-md hover:border-primary-200 dark:hover:border-primary-800/50 transition-all duration-300 cursor-pointer overflow-hidden"
                onClick={() => setSelectedQuestion(isExpanded ? null : question._id)}
              >
                <div className="p-5">
                  <div className="flex items-start justify-between">
                    <div className="flex items-start gap-4 flex-1">
                      <div className="w-11 h-11 bg-gradient-to-br from-primary-500/10 to-primary-500/10 dark:from-primary-500/20 dark:to-primary-500/20 rounded-xl flex items-center justify-center flex-shrink-0 border border-primary-100 dark:border-primary-800/30">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center flex-wrap gap-2 mb-2">
                          ...
                          <span className="inline-flex items-center px-2.5 py-1 bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-400 rounded-lg text-xs font-bold">
                            {question.category}
                          </span>
                        ...
                        {question.tags && question.tags.length > 0 && (
                          <div className="flex flex-wrap gap-1.5 mt-2.5">
                            {question.tags.map((tag, index) => (
                              <span
                                key={index}
                                className="text-xs text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-lg font-medium"
                              >
                                {tag}
                              </span>
                            ))}
                    ...
                    <button className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-primary-600 dark:text-primary-400 hover:bg-primary-50 dark:hover:bg-primary-900/20 rounded-lg transition-colors flex-shrink-0 ml-3">
            ...
            <button
              ...
              className={`w-10 h-10 rounded-xl font-semibold text-sm transition-all duration-200 ${
                i + 1 === data.pagination.page
                  ? 'bg-gradient-to-r from-primary-600 to-primary-600 text-white shadow-lg shadow-primary-500/25'
                  : 'bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:border-primary-300 dark:hover:border-primary-700 hover:text-primary-600 dark:hover:text-primary-400'
              }`}
            >
              {i + 1}
            </button>
