...
const DEFAULT_TOPICS = [
  ...
  },
  {
    ...
    category: 'Technical',
    emoji: '🏗️',
    subtopics: ['Scalability', 'Load Balancing', 'Caching', 'Database Design', 'Microservices'],
  },
  {
    id: '3',
    ...
    emoji: '💬',
    subtopics: ['STAR Method', 'Leadership', 'Conflict Resolution', 'Teamwork', 'Failure Stories'],
  },
  {
    id: '4',
    ...
    category: 'Technical',
    emoji: '⚛️',
    subtopics: ['Closures', 'Promises', 'Hooks', 'State Management', 'Performance'],
  },
  {
    id: '5',
    title: 'SQL & Databases',
    category: 'Technical',
    emoji: '🗄️',
    subtopics: ['Joins', 'Indexing', 'Normalization', 'Transactions', 'NoSQL'],
  },
  {
    id: '6',
    ...
    category: 'Soft Skills',
    emoji: '🎙️',
    subtopics: ['Clarity', 'Active Listening', 'Presentation', 'Body Language'],
  },
];
...
const StudyPlan = () => {
  ...
  const persistState = useCallback(
    (newTopics, newStreak, newDate) => {
      const state = {
        topics: newTopics || topics,
        streak: newStreak ?? streak,
        lastStudyDate: newDate || lastStudyDate,
      };
      savePlan(state);
    },
    ...
    const updated = topics.map(t => {
      ...
      const subs = t.subtopics.map((s, i) => {
        ...
        return {
          ...
          lastStudied: !s.completed ? new Date().toISOString() : s.lastStudied,
        };
      });
  ...
  const filteredTopics =
    filter === 'all'
      ? topics
      : filter === 'incomplete'
        ? topics.filter(t => t.subtopics.some(s => !s.completed))
  ...
  const getCategoryGradient = category => {
    const gradients = {
      Technical: 'from-blue-500 to-primary-600',
    ...
    <div className="max-w-6xl mx-auto space-y-8">
        ...
        <Card className="overflow-hidden">
          ...
          <div className="p-6">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-5">
              Create a new topic to track your learning progress
            </p>
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none placeholder:text-slate-400 dark:placeholder:text-slate-500"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <Card
            key={stat.label}
            className={`${stat.bg} border ${stat.border} relative overflow-hidden`}
          >
            <div className="flex items-center gap-3">
              <div
                className={`w-11 h-11 bg-gradient-to-br ${stat.gradient} rounded-xl flex items-center justify-center shadow-lg shrink-0`}
              >
                <stat.icon className="w-5 h-5 text-white" />
              </div>
              <div className="min-w-0">
                <p className={`text-2xl font-bold ${stat.text}`}>{stat.value}</p>
                <p className={`text-xs font-medium ${stat.text} opacity-80 truncate`}>
                  {stat.label}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  These subtopics were last studied over 3 days ago — time for a refresh!
                </p>
              </div>
            </div>
            <div className="flex flex-wrap gap-2 mt-4">
              {needsReview.slice(0, 12).map((item, i) => (
                ...
                >
                  ...
                  <span className="text-amber-500 dark:text-amber-500 font-semibold">
                    {item.topic}:
                  </span>{' '}
                  {item.subtopic}
                </span>
      ...
      <div className="space-y-5">
        {filteredTopics.map(topic => {
          ...
          return (
            <Card
              key={topic.id}
              className={`relative overflow-hidden transition-all duration-300 ${isDone ? 'ring-2 ring-emerald-200 dark:ring-emerald-800' : ''}`}
            >
              {/* Accent bar */}
              <div
                className={`absolute top-0 left-0 right-0 h-1 bg-gradient-to-r ${getCategoryGradient(topic.category)}`}
              />
              ...
              <div className="pt-1">
                ...
                >
                  <div className="flex items-center gap-4 min-w-0">
                    ...
                    <div className="min-w-0">
                      <h3 className="font-bold text-slate-900 dark:text-white text-lg group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors truncate">
                        {topic.title}
                      </h3>
                    ...
                    <div className="relative w-12 h-12 hidden sm:flex items-center justify-center">
                      <svg className="w-12 h-12 -rotate-90" viewBox="0 0 36 36">
                        ...
                        <circle
                          ...
                          strokeLinecap="round"
                          className={isDone ? 'stroke-emerald-500' : 'stroke-primary-500'}
                        />
                      </svg>
                      <span
                        className={`absolute text-[10px] font-bold ${isDone ? 'text-emerald-600 dark:text-emerald-400' : 'text-primary-600 dark:text-primary-400'}`}
                      >
                        {progress}%
                      </span>
                    </div>
                ...
                <div className="mt-3 sm:hidden">
                  <div className="h-2 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full transition-all duration-500 ${isDone ? 'bg-gradient-to-r from-emerald-500 to-teal-500' : 'bg-gradient-to-r from-primary-500 to-primary-500'}`}
                      ...
                      {topic.subtopics.map((sub, i) => (
                        ...
                        >
                          {sub.completed ? (
                            ...
                            <Circle className="w-5 h-5 text-slate-300 dark:text-slate-600 group-hover/sub:text-primary-400 shrink-0 transition-colors" />
                          )}
                          <span
                            className={`font-medium ${sub.completed ? 'text-emerald-700 dark:text-emerald-400 line-through opacity-70' : 'text-slate-700 dark:text-slate-300'}`}
                          >
                            {sub.name}
                          </span>
                          {sub.completed && sub.lastStudied && (
                            <span className="ml-auto text-[10px] text-slate-400 dark:text-slate-500 shrink-0">
                              {new Date(sub.lastStudied).toLocaleDateString(undefined, {
                                month: 'short',
                                day: 'numeric',
                              })}
                            </span>
