...
const MarkdownText = ({ text }) => {
  const renderLine = line => {
    ...
    html = html.replace(
      /`([^`]+)`/g,
      '<code class="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-700 rounded text-xs font-mono text-primary-600 dark:text-primary-400">$1</code>'
    );
    html = html.replace(
      /\[([^\]]+)\]\(([^)]+)\)/g,
      '<a href="$2" target="_blank" rel="noopener" class="text-primary-500 underline hover:text-primary-600">$1</a>'
    );
    return html;
  };
        ...
        <ul key={`ul-${elements.length}`} className="list-disc list-inside space-y-1 my-2 pl-2">
          {listItems.map((li, i) => (
            <li
              ...
              dangerouslySetInnerHTML={{ __html: renderLine(li) }}
            />
          ))}
        </ul>
      );
  ...
  lines.forEach((line, i) => {
    if (line.startsWith('```')) {
      if (inCodeBlock) {
        elements.push(
          <div
            key={`code-${i}`}
            className="my-3 rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700"
          >
            {codeLang && (
    ...
    if (line.startsWith('### ')) {
      elements.push(
        <h4 key={i} className="font-bold text-base mt-3 mb-1 text-slate-900 dark:text-white">
          {line.slice(4)}
        </h4>
      );
  ...
  if (inCodeBlock && codeLines.length) {
    elements.push(
      <pre
        key="code-end"
        className="bg-slate-50 dark:bg-[#0d1526] rounded-xl p-4 my-3 overflow-x-auto text-sm font-mono border border-slate-200 dark:border-slate-700"
      >
        <code>{codeLines.join('\n')}</code>
      </pre>
    );
...
const CONVERSATION_MODES = [
  ...
  },
  {
    ...
    prompt:
      'You are an interviewer. Ask me technical interview questions one by one, wait for my answer, evaluate it, and then ask the next question. Start with a brief introduction.',
  },
  {
    id: 'dsa',
    label: 'DSA Helper',
    ...
    prompt:
      'You are a DSA expert. Help me solve coding problems step by step. Explain time/space complexity. Give hints before full solutions.',
  },
  {
    id: 'resume',
    label: 'Resume Review',
    ...
    prompt:
      'You are a resume expert. Help me improve my resume for tech roles. Ask me about my experience and give specific actionable feedback.',
  },
  {
    id: 'behavioral',
    ...
    prompt:
      'You are a behavioral interview coach. Help me prepare STAR method answers. Ask me behavioral questions and coach me on my responses.',
  },
  {
    id: 'system',
    label: 'System Design',
    ...
    prompt:
      'You are a system design expert. Help me practice system design interviews. Ask me to design systems and guide me through the process.',
  },
];
...
const AIChatPage = () => {
  ...
  const generateAIResponse = async userMessage => {
    ...
    try {
      ...
      return (
        ...
        "I'm here to help! Try asking about interview tips, DSA, or career guidance."
      );
    } catch (error) {
  ...
  const getFallbackResponse = msg => {
    ...
    if (lower.includes('interview') || lower.includes('tip'))
  ...
  const exportChat = () => {
    const text = messages
      .map(m => `[${m.type === 'bot' ? 'AI' : 'You'}] ${m.text}`)
      .join('\n\n---\n\n');
  ...
  return (
    <div className="flex h-[calc(100vh-7rem)] gap-4">
        ...
        <div className="p-4 border-b border-slate-100 dark:border-slate-700/50">
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-primary-600 to-primary-600 hover:from-primary-700 hover:to-primary-700 text-white rounded-xl font-medium transition-all shadow-lg shadow-primary-500/25"
            >
      ...
      <div className="flex-1 flex flex-col bg-white dark:bg-[#111827] rounded-2xl border border-slate-200 dark:border-slate-700/50 overflow-hidden">
        {/* Header */}
        <div className="bg-gradient-to-r from-primary-600 via-primary-600 to-primary-600 px-6 py-4 text-white flex-shrink-0">
              ...
              >
                ...
                >
                  ...
                  <div className="flex items-center justify-between mt-2 pt-2 border-t border-slate-200/50 dark:border-slate-600/30">
                    <span
                      className={`text-xs ${message.type === 'user' ? 'text-primary-200' : 'text-slate-400'}`}
                    >
          ...
          {isTyping && (
            <div className="flex justify-start">
              <div className="flex items-start gap-3">
                <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-primary-500 to-pink-500 flex items-center justify-center text-white">
                  <Bot className="w-5 h-5" />
                </div>
            ...
            <div className="flex flex-wrap gap-2">
              {QUICK_PROMPTS.map((prompt, i) => (
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-2 bg-white dark:bg-slate-800 hover:bg-primary-50 dark:hover:bg-primary-900/20 hover:text-primary-600 dark:hover:text-primary-400 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-medium text-slate-600 dark:text-slate-300 transition-all hover:border-primary-300 dark:hover:border-primary-500/30"
                >
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-700/50 bg-white dark:bg-[#111827] flex-shrink-0">
          <div className="flex items-end gap-3">
            ...
            <div className="flex-1 relative">
              <textarea
                ...
                placeholder={
                  isListening
                    ? 'Listening...'
                    : 'Ask me anything about interviews, coding, career...'
                }
                rows={1}
                className="w-full px-4 py-3 pr-12 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent text-sm bg-slate-50 dark:bg-slate-800/50 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 resize-none"
            ...
            <button
              ...
              className="p-3 bg-gradient-to-r from-primary-600 to-primary-600 text-white rounded-xl hover:from-primary-700 hover:to-primary-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-primary-500/25 flex-shrink-0"
            >
              {isTyping ? (
