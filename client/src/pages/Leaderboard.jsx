...
const Leaderboard = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await api.get(
        `/analytics/leaderboard?timeRange=${timeRange}&category=${category}`
      );
  ...
  return (
    <div className="space-y-8">
      ...
      {userRank && (
        <Card className="bg-gradient-to-r from-primary-50 to-primary-50 dark:from-primary-900/20 dark:to-primary-900/20 border-primary-200 dark:border-primary-800">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent text-slate-900 dark:text-slate-100 placeholder-slate-400"
          />
        </div>
        <select
          value={timeRange}
          onChange={e => setTimeRange(e.target.value)}
          className="px-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent text-slate-900 dark:text-slate-100"
        >
      ...
      {leaderboardData.length === 0 && !isLoading ? (
        <Card className="text-center py-16">
          ...
          <p className="text-slate-500 dark:text-slate-400 max-w-md mx-auto">
            Complete at least 5 interviews to appear on the leaderboard. Start practicing to climb
            the ranks!
          </p>
        </Card>
      ) : (
              ...
              <div className="w-full sm:flex-1 max-w-[280px] sm:max-w-[220px] order-1 sm:order-2">
                <Card className="relative overflow-hidden text-center bg-gradient-to-b from-yellow-50 to-amber-100 dark:from-yellow-900/30 dark:to-amber-900/30 border-yellow-300 dark:border-yellow-700 shadow-xl shadow-yellow-500/20">
                  ...
                  <div className="p-5 pt-6">
                    ...
                    <h3 className="font-bold text-slate-900 dark:text-slate-100 text-lg truncate mt-1">
                ...
                <div className="h-24 bg-gradient-to-b from-yellow-300 to-amber-400 dark:from-yellow-700 dark:to-amber-800 rounded-b-xl mx-2 hidden sm:flex items-center justify-center">
                  <span className="text-3xl font-black text-yellow-600 dark:text-yellow-300">
                    1
                  </span>
                </div>
              </div>
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-center justify-between p-4 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors ${getRankBg(user.rank)} border-l-4 ${
                          isCurrentUser
                            ? 'ring-2 ring-primary-500 ring-inset bg-primary-50/50 dark:bg-primary-900/10'
                            : ''
                        }`}
                      >
                        <div className="flex items-center space-x-2 sm:space-x-4 min-w-0">
                          ...
                          <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-gradient-to-br from-primary-400 to-primary-500 flex items-center justify-center text-white font-bold shadow-md overflow-hidden flex-shrink-0">
