...
const MockInterviewLab = () => {
  ...
  return (
    <div className="space-y-8 animate-slide-up">
      {/* Header */}
      <div className="relative overflow-hidden bg-gradient-to-r from-slate-900 via-primary-900 to-slate-900 rounded-3xl p-8 text-white shadow-2xl">
        {/* Animated background elements */}
        <div className="absolute inset-0 overflow-hidden">
          <div className="absolute top-0 left-1/4 w-96 h-96 bg-primary-500/20 rounded-full blur-3xl animate-pulse-slow" />
          <div
            className="absolute bottom-0 right-1/4 w-72 h-72 bg-blue-500/20 rounded-full blur-3xl animate-pulse-slow"
            style={{ animationDelay: '1s' }}
          />
          {/* Floating code snippets */}
          <div className="absolute top-10 right-20 text-primary-300/20 font-mono text-xs animate-float">
            {'{ code: "perfect" }'}
          </div>
          <div
            className="absolute bottom-10 left-20 text-blue-300/20 font-mono text-xs animate-float"
        ...
        <div className="relative flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          <div>
            ...
            <p className="text-primary-200 text-lg max-w-xl">
              Practice in a realistic interview environment with AI-powered feedback and real-time
              coding challenges.
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-3 sm:gap-4">
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 border border-white/10">
              <div className="text-2xl font-bold">
                {Math.round((statsData?.totalPracticeTime || 0) / 60)}h
              </div>
              <div className="text-sm text-primary-200">Practice</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {interviewModes.map(mode => (
            ...
            >
              ...
              <div
                className={`absolute inset-0 bg-gradient-to-br ${mode.gradient} opacity-0 group-hover:opacity-5 transition-opacity duration-300`}
              />

              <div className="relative">
                <div
                  className={`w-14 h-14 bg-gradient-to-br ${mode.gradient} rounded-2xl flex items-center justify-center shadow-lg ${mode.shadow} mb-4 group-hover:scale-110 transition-transform duration-300`}
                >
                ...
                <div className="flex items-center justify-between text-sm text-slate-400">
                  ...
                  <ChevronRight
                    className={`w-5 h-5 transition-transform duration-300 ${selectedMode === mode.id ? 'translate-x-1' : ''}`}
                  />
                </div>
              </div>
            </Card>
      ...
      {/* Start Session Panel */}
      {selectedMode && (
        <Card className="bg-gradient-to-r from-slate-50 to-white border-2 border-dashed border-primary-200 animate-scale-in">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 bg-gradient-to-br from-primary-500 to-primary-500 rounded-2xl flex items-center justify-center shadow-lg shadow-primary-500/30 animate-pulse-slow">
              ...
              <div>
                ...
                <p className="text-slate-500 dark:text-slate-400">
                  {interviewModes.find(m => m.id === selectedMode)?.title} -{' '}
                  {interviewModes.find(m => m.id === selectedMode)?.duration}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <Button size="lg" icon={Play} onClick={handleStartInterview} disabled={isStarting}>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4">
                        <div className="text-right">
                          <div
                            className={`text-xl font-bold ${session.score >= 85 ? 'text-green-500' : session.score >= 70 ? 'text-amber-500' : 'text-red-500'}`}
                          >
                            {session.score}%
                          </div>
                ...
                <div
                  ...
                  className={`p-4 rounded-xl text-center transition-all duration-300 ${
                    achievement.unlocked
                      ? 'bg-gradient-to-br from-primary-50 to-primary-50 border border-primary-100'
                ...
                >
                  <div
                    className={`w-10 h-10 mx-auto mb-2 rounded-xl flex items-center justify-center ${
                      achievement.unlocked
                        ? 'bg-gradient-to-br from-primary-500 to-primary-500 shadow-lg shadow-primary-500/30'
                        : 'bg-slate-200'
                    }`}
                  >
                    <achievement.icon
                      className={`w-5 h-5 ${achievement.unlocked ? 'text-white' : 'text-slate-400'}`}
                    />
                  </div>
                  <p
                    className={`text-sm font-medium ${achievement.unlocked ? 'text-slate-900 dark:text-white' : 'text-slate-400'}`}
                  >
                    {achievement.label}
                  </p>
                </div>
      ...
      {/* Tips Section */}
      <Card className="bg-gradient-to-r from-primary-50 to-primary-50 border-none">
        <div className="flex items-start gap-4">
          <div className="w-12 h-12 bg-gradient-to-br from-primary-500 to-primary-500 rounded-xl flex items-center justify-center shadow-lg shadow-primary-500/30 flex-shrink-0">
            <Lightbulb className="w-6 h-6 text-white" />
          </div>
          <div>
            <h3 className="font-bold text-slate-900 dark:text-white mb-1">Pro Tip</h3>
            <p className="text-slate-600 dark:text-slate-400">
              Start with Speed Round to warm up, then move to Deep Dive sessions for more thorough
              practice. Consistency is key - try to complete at least one session daily!
            </p>
          </div>
        </div>
      </Card>
